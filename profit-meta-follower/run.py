"""Run the profit-meta follower.
  ...
  $env:PMF_PROFILE="cloud"; python profit-meta-follower/run.py
  $env:PMF_PROFILE="research"; python profit-meta-follower/run.py

Profiles use separate data folders (data-local / data-cloud / data-research).
"""
...
def main() -> None:
    ...
    if research_only:
        logger.info("RESEARCH mode — no live orders; gathering crowd books + marks only")
    elif not bool(cfg.PAPER_TRADING):
